...
| 前端 | React 18 / TypeScript / Vite 8 / Vitest |
...
当前本地验收状态：后端 97 个测试、前端 37 个测试全部通过，`npm run build`
成功。
...
Install the backend in editable mode and install the locked frontend
dependencies:
...
The `cifar10` backend preset uses `torchvision.datasets.CIFAR10` with
`download=False`; the loader therefore expects the extracted directory
`data/cifar-10-batches-py/`. CIFAR-10 is provided by the official University
of Toronto archive and is the dataset source used by torchvision. From the
repository root, prepare it with:

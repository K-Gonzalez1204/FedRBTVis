...
def audit_repository(root: Path) -> tuple[list[str], list[str]]:
    ...
    if tracked is not None:
        ...
        for relative in large:
            if relative.as_posix() == "report.pdf":
                warnings.append("PUBLIC_REVIEW_REQUIRED: report.pdf")
            else:
                issues.append(f"oversized tracked file: {relative.as_posix()}")

    if (root / "report.pdf").exists():
        warnings.append("PUBLIC_REVIEW_REQUIRED: report.pdf")
    warnings.append("G5_BLOCKED: license and clean-history export not selected")
